			...
			for qh in range(4):
				...
				for i in range(6):
					...
					if result == 0:
						...
						data_new[row].append(0)
					elif result == 3:

						data_new[row].append(400)
					else:
						
						if data[line][result+1] > 400:
							print(line)
							data_new[row].append(400)
						else:
							data_new[row].append(data[line][result+1])
					line += 1
...
cbar = plt.colorbar(c)#,ticks = np.linspace(65,200,15))
